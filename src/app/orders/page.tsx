...
import { getAllGivenOrders, getAllOrders } from "@/actions/read";
...
export default function OrdersPage() {
	...
	const [givenOrders, setGivenOrders] = useState<OrderType[]>([]);

	useEffect(() => {
		const fetchOrders = async () => {
			const orders = await getAllOrders();
			const givenOrders = await getAllGivenOrders();

			setGivenOrders(givenOrders);
			setOrders(orders);
		};
		fetchOrders();
	}, []);
	...
	return (
		<>
			...
			<div className="container mx-auto p-6">
				...
				<h1 className="text-4xl font-bold mb-6 mt-9">
					Выданные заказы
				</h1>
				<table className="min-w-full border border-gray-300">
					<thead className="bg-gray-100">
					...
					<tbody>
						{givenOrders.map((order: OrderType) => (
							<tr key={order.id} className="hover:bg-gray-50">
								<td className="border px-4 py-2">{order.id}</td>
								<td className="border px-4 py-2">
									{order.customer.name}{" "}
